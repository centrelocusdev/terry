...
const CreateContact = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const res = await createContact({...basicInfo, billing_address: billingAddress})
    if(res) {
      const contact_id = res?.contact.contact_id
      const user_res = await getUserByToken()
      const insert = await insertContact(user_res.email, contact_id)
    ...
    <>
      ...
      <div className="bg-white md:p-16 p-5 rounded mt-5">
        <div onChange={onChangeBasicInfo}>
          ...
          <div className="md:flex gap-5 justify-between mt-4">
            <InputPrimary label={"twitter"} name={"twitter"} placeholer={""} />
            <InputPrimary
              label={"facebook"}
              name={"facebook"}
              placeholer={""}
          ...
          <div className="md:flex gap-5 justify-between mt-4">
            <InputPrimary label={"website"} name={"website"} placeholer={""} />
          </div>
        </div>
        ...
        <div onChange={onChangeBillingAddress} className="mt-5">
          ...
          <div className="md:flex gap-5 justify-between mt-4">
            <InputPrimary
              ...
              placeholer={""}
            />
            <InputPrimary label={"address"} name={"address"} placeholer={""} />
          </div>
          <div className="md:flex gap-5 justify-between mt-4">
            <InputPrimary
              label={"country"}
